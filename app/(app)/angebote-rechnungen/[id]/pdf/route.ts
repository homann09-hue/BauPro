...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await checkRateLimit(`commercial-document-pdf:${context.companyId}:${context.userId}:${getClientIp(request.headers)}`, 40, 60_000);
    ...
    const pdf = buildCommercialDocumentPdf(data);

    return new Response(new Uint8Array(pdf), {
      headers: downloadHeaders("application/pdf", commercialDocumentFilename(data))
    });
  } catch (error) {
