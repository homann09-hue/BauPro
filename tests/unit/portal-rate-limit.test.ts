...
describe("portal IP rate limiting", () => {
  ...
  it("blockiert die 31. Portal-Anfrage innerhalb einer Minute pro IP", async () => {
    ...
    for (let index = 0; index < 30; index += 1) {
      assertRateLimit(key, 30, 60_000);
    }

    expect(() => assertRateLimit(key, 30, 60_000)).toThrow("Zu viele Anfragen");
  });
  ...
  it("limitiert verschiedene IPs unabhaengig voneinander", async () => {
    ...
    for (let index = 0; index < 30; index += 1) {
      assertRateLimit("portal-view:203.0.113.10", 30, 60_000);
      assertRateLimit("portal-view:198.51.100.2", 30, 60_000);
    }

    expect(() => assertRateLimit("portal-view:203.0.113.10", 30, 60_000)).toThrow("Zu viele Anfragen");
    expect(() => assertRateLimit("portal-view:198.51.100.2", 30, 60_000)).toThrow("Zu viele Anfragen");
  });
  ...
  it("macht Rate-Limit und ungueltige Tokens nicht ueber unterschiedliche Portal-Antworten unterscheidbar", () => {
    ...
    expect(page).toContain("assertRateLimit(`portal-view:${clientIp}`, 30, 60_000)");
    ...
    expect(pdfRoute).toContain("assertRateLimit(`portal-pdf:${clientIp}`, 30, 60_000)");
